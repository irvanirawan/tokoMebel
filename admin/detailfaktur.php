...
// a
  for ($i=0; $i < count($b); $i++) {
    ...
    $aarray=$a[$i];
    $total=$darray*$aarray;
    $qupj="update detail_faktur set jumlah=$aarray, total=$total where id_detail_faktur=$barray";
...
customer_barang.harga_jual, detail_faktur.total, barang.nama_barang, faktur.id_faktur, detail_faktur.id_detail_faktur, detail_faktur.modal, 
faktur.status FROM detail_faktur INNER JOIN barang ON detail_faktur.id_barang = barang.id_barang 
...
while($d=mysqli_fetch_array($e)){ ?>
  <tr>
    ...
    <td style="text-align: center"><?php echo $d['jumlah']; ?><?php if($d['total']==1){echo readonly;} ?></td>
...
<div class="container-fluid form-outline nav navbar-left">
	<span><a href="tambahbarang.php?id=<?php echo $dqueryfaktur['id_faktur'];?>" 
...
<div class="container-fluid form-outline nav navbar-right">	
	<span><input type="submit" name="print" value="Print Invoice"
